...
//Added by Suraj on 24 APRIL 2018 for close file review
function ValidateCloseFileReviewBlog() {
    debugger;
    var flag = true;
    debugger;
    $("#grdclosefilerw tr:has(td)").each(function () {

        var ddlAnswer = $(this).find('#ddlAnswer').val();

        if (ddlAnswer == "0") {
            $(this).find('#ddlAnswer').addClass('EmptyVal');
            flag = false;
        }
        else {
            $(this).removeClass('EmptyVal');
    ...
    $('#divCloseFileReview').find('textarea').each(function () {
    ...
    $('#divCloseFileReview').find('.validate').each(function () {
        debugger;
        var control = $.trim($(this).val());
        if (control == "0") {
            debugger;
...
function ValidateMasterPostBlog() {
    ...
    var ddlUWreason = $("#ddlUWreason option:selected").val();
    var ddlPeriod = $("#ddlPeriod option:selected").val();
    //Added by Suraj on 25 APRIL 2018 for Assignment type
    var ddlApplicationDetailsProposalType = $("#ddlApplicationDetailsProposalType option:selected").val();
    var ddlAssigmentType = $("#ddlAssigmentType option:selected").val();
    //alert(ddlUWDecesion.value);

    //added by Suraj on 29/06/2021 for CR=30450 - add business exception radio button
    if (ddlUWDecesion == "Approved") {
        //alert(ddlUWDecesion);
    ...
    if (ddlApplicationDetailsProposalType == "0") {
        $('#lblErrorinfo').html("Please select Proposal Type before Post Data");
        $('#ddlApplicationDetailsProposalType').addClass('EmptyVal');
        $('#UpdatePanel13').find(':input:enabled:visible:first').focus();
        Flag = false;
    }
    //if (ddlAssigmentType == "EMP") {
    //    $('#lblErrorinfo').html("Please select Assignment Type before Post Data");
    //    $('#ddlAssigmentType').addClass('EmptyVal');
    //    $('#UpdatePanel13').find(':input:enabled:visible:first').focus();
    //    Flag = false;
    //}
    //End
//Added By Royson for Validation for Restrict Further Cover
   if (ddlUWDecesion != "proposal") {
       if($("input:radio[name='ctl00$ContentPlaceHolder1$restrict_further_cover']").length!=0){
       if(!$("input:radio[name='ctl00$ContentPlaceHolder1$restrict_further_cover']").is(":checked")){
        $('#lblErrorinfo').html("Please select if you want to restrict further cover");
        Flag = false;
       }
    }
    }
