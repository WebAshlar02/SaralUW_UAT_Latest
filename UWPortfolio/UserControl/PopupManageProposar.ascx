...
<script>
    ...
    function fnFillClientControls(ClientDetails) {
        ...
        // 1.1 Begin of changes CR-4971 Sushant Devkate -MFL00905

        if (ClientDetails.ClientType == "P" && ((ClientDetails.AssuredType != "Nominee") && (ClientDetails.AssuredType != "Appointee"))) {
            $('#divchkIsNRICustomer').removeClass('HideControl');
        } else {
            $('#divchkIsNRICustomer').addClass('HideControl');
        }

        $('#chkIsNRICustomer').prop('checked', ClientDetails.IsNRICustomer);
        if ($('#chkIsNRICustomer').is(':checked')) {
            $("#txtCommuPhone1").removeProp("maxlength", "");
            $("#txtCommuPhone1").prop("maxlength", "50");
        }
        else
        {
            $("#txtCommuPhone1").removeProp("maxlength", "");
            $("#txtCommuPhone1").prop("maxlength", "12");
        }

        // 1.1 End of changes CR-4971 Sushant Devkate -MFL00905
        ...
        if ($("#cbNationality option[value='" + ClientDetails.Nationality + "']").length > 0) {
            ...
            $('#hdnNationlity').val((ClientDetails.Nationality == '') ? '-1' : ClientDetails.Nationality);
        ...
        if ($('input[name="Role"]:checked').val().toLowerCase() != 'nominee') {
            ...
            if ($('#chkIsNRICustomer').is(':checked')) {
                if ($('#txtCommuEmailId').val() == '') {
                    $('#txtCommuEmailId').parent().find('.lblError').removeClass('HideControl').html('This is an NRI customer and email ID is mandatory');
                ...
                if ($('#txtCommuEmailId').val() != '') {
                    if (!isValidEmailAddress($('#txtCommuEmailId').val())) {
                        $('#txtCommuEmailId').parent().find('.lblError').removeClass('HideControl').html('Enter Valid Email Id');
                        $('#txtCommuEmailId').focus();
                        return false;
                    }
                }
    ...
    function fnSaveCreateClient() {
        if (fnValidateClientDetails("1")) {
            ...
            var response = UserControl_PopupManageProposar.CreateClientInfo($('#txtLaClientId').val(), $('input[name="Role"]:checked').val(), $('#cbSalutation :checked').val(), $('#txtLaFirstName').val(), $('#txtLaMiddleName').val()
                ...
                , $('#txtCommuCountryCode :checked').val(), $('#txtCommuAddressRemark').val(), $('#txtCommuPhone1').val(), $('#txtCommuPhone2').val(), $('#txtCommuTelNo').val()
                , $('#txtCommuMobileNo').val(), $('#txtCommuEmailId').val(), $('#txtCommuFaxNo').val(), $('#cbNationality :checked').val(), $('#chkIsNSAP').is(':checked')
                , $('#cbOccupation :checked').val(), strApplnNo, strChannelType, $('#txtOldValue').val(), // 1.1 Begin of changes CR-4971 Sushant Devkate -MFL00905
                $('#chkIsNRICustomer').is(':checked')
                // 1.1 End of changes CR-4971 Sushant Devkate -MFL00905
            );
            var splits = response.value.split('#');
...
<div class="box box-warning box-solid divClient HideControl">
    <div class="row"></div>
    <input type="hidden" id="hdnNationlity" runat="server" />
    <div class="box-header with-border">
        <div class="col-md-12">
            <div class="col-md-9">
    ...
    <div class="box-body box-bodyPopUp details">
        ...
        <div class="col-md-12">
            ...
            <!--2.1 Begin Of Changes added style="display:none; by Sushant Devkate [MFL00905] -->
            ...
            <div class="col-md-4 HideControl" id="divchkIsNRICustomer">
                <div class="col-md-12">
                    <label>IsNRICustomer</label>
                </div>
                <div class="form-group">
                    <input class="cbEnableDisable" disabled="true" type="checkbox" id="chkIsNRICustomer" onchange="FnNRICustomer();" style="width: 24px; height: 19px;" />
                </div>
            </div>
            ...
            <div class="col-md-4 communication">
                <div class="form-group">
                    <label>Phone 1</label>
                    <input type="text" id="txtCommuPhone1" class="form-control lblLable Numeric" maxlength="12" />
                </div>
            </div>
            <div class="col-md-4 communication">
                <div class="form-group">
